...
module.exports = function token(options, issue) {
  ...
  function response(txn, res, next) {
    ...
    function issued(err, accessToken, params) {
      ...
      // AKL Changed the redirect since something internal to it changes https to http
      // ... also some confusion about what is passed, this states http.ServerResponse
      // ... however ServerResponse does not have a redirect() method according to the
      // ... node doucmentation.
      //return res.redirect(location);
      res.statusCode = 303;
